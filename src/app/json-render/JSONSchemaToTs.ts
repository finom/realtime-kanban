...
export function JSONSchemaToTs(
  jsonSchema: VovkJSONSchemaBase | boolean,
): string {
  ...
  if (jsonSchema.enum) {
    return (
      jsonSchema.enum.map((v: any) => JSON.stringify(v)).join(" | ") || "never"
    );
  }

  // Handle allOf (intersection)
  if (jsonSchema.allOf) {
    const parts = jsonSchema.allOf.map((s: any) => JSONSchemaToTs(s));
    return parts.length ? `(${parts.join(" & ")})` : "unknown";
  }

  // Handle anyOf (union)
  if (jsonSchema.anyOf) {
    const parts = jsonSchema.anyOf.map((s: any) => JSONSchemaToTs(s));
    return parts.length ? `(${parts.join(" | ")})` : "never";
  }

  // Handle oneOf (union)
  if (jsonSchema.oneOf) {
    const parts = jsonSchema.oneOf.map((s: any) => JSONSchemaToTs(s));
    return parts.length ? `(${parts.join(" | ")})` : "never";
  }
  ...
  if (Array.isArray(jsonSchema.type)) {
    const types = jsonSchema.type.map((t: string) =>
      JSONSchemaToTs({ ...jsonSchema, type: t as VovkJSONSchemaBase["type"] }),
    );
  ...
  if (
    ...
    if (jsonSchema.additionalProperties === true) {
      ...
      additionalType = JSONSchemaToTs(jsonSchema.additionalProperties);
    }
  ...
  if (type === "array" || jsonSchema.items || jsonSchema.prefixItems) {
    ...
    if (jsonSchema.prefixItems) {
      const tupleTypes = jsonSchema.prefixItems.map((s: any) =>
        JSONSchemaToTs(s),
      );
      if (jsonSchema.items === false) {
        return `[${tupleTypes.join(", ")}]`;
      }
      const restType = jsonSchema.items
        ? JSONSchemaToTs(jsonSchema.items)
        : "unknown";
    ...
    if (jsonSchema.items) {
      if (Array.isArray(jsonSchema.items)) {
        // Legacy tuple syntax
        const tupleTypes = jsonSchema.items.map((s: any) => JSONSchemaToTs(s));
        if (jsonSchema.additionalItems === false) {
          return `[${tupleTypes.join(", ")}]`;
        }
        const restType = jsonSchema.additionalItems
          ? JSONSchemaToTs(jsonSchema.additionalItems)
          : "unknown";
        return `[${tupleTypes.join(", ")}, ...${restType}[]]`;
      }
      return `${JSONSchemaToTs(jsonSchema.items)}[]`;
    }
  ...
  if (jsonSchema.properties) {
    return JSONSchemaToTs({ ...jsonSchema, type: "object" });
  }
